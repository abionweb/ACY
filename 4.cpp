...
int main() {
	...
	if ( K + 1 ) {
		cout << "Удаляю M[" << K << "]" << endl;
		for (i = K; i < n; i++)	M[i] = M[i + 1];
		--n;
	} else {
		cout << "Нет элемента равного 0" << endl;
	}
	...
	cout << "Добавляю после каждого четного элемента массива элемент со значением M[ I-1 ]+2" << endl;
	for (i = n-1; i > 1; i--) {
		if (  i % 2 == 0 ) {
	...
	// Задание 5: Распечатать полученный массив.
	for (i = 0; i < n; i++) cout << "M[" << i << "]: " << M[i] << endl;
	cout << endl;
}
